...
import { useWorkspaceStore } from "@/stores/useWorkspaceStore"; // Assuming AssociationNodeData will be added here
...
import { AssociationNodeData } from "@/types";
...
export function AssociationNode({
  ...
  const [pendingData, setPendingData] =
    useState<Partial<AssociationNodeData>>(data);
  ...
  const handleInputChange = useCallback(
    (field: keyof AssociationNodeData, value: string | undefined) => {
  ...
  const handleCancelChanges = useCallback(() => {
    ...
    setIsEditing(false);
    if (!data.fileId) {
      // If was a new node and cancelled, perhaps remove it or revert to placeholder
  ...
  const columnOptions =
    selectedFileMeta?.columns?.map((col) => ({ value: col, label: col })) || [];

  // Check if pending changes are different from original data
  const hasChanges = JSON.stringify(pendingData) !== JSON.stringify(data);
  const canApply =
    pendingData.fileId &&
    pendingData.inputIndexColumn &&
    pendingData.dataColumn &&
    pendingData.aggregation &&
    pendingData.outputColumnName &&
    pendingData.outputColumnName.trim() !== "";
  ...
  return (
    <Card className="w-80 border border-blue-200 bg-blue-50 shadow-md">
      <NodeResizer
        minWidth={320}
      ...
      />
      <CardHeader className="flex flex-row items-center justify-between border-b border-blue-200 bg-blue-100 p-2">
      ...
      {isEditing ? (
        ...
        <CardContent className="space-y-2 p-3">
          ...
          <div>
            ...
            <Select
              value={pendingData.fileId || ""}
              onValueChange={(value) => handleInputChange("fileId", value)}
            >
          ...
          {pendingData.fileId && selectedFileMeta && (
            <>
              <div>
                ...
                <Select
                  value={pendingData.inputIndexColumn || ""}
                  onValueChange={(value) =>
                    handleInputChange("inputIndexColumn", value)
                  }
                >
                  <SelectTrigger className="h-8 text-xs">
                    <SelectValue placeholder="选择关联列..." />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div>
                ...
                <Select
                  value={pendingData.dataColumn || ""}
                  onValueChange={(value) =>
                    handleInputChange("dataColumn", value)
                  }
                >
                  <SelectTrigger className="h-8 text-xs">
              ...
              <div>
                ...
                <Select
                  value={pendingData.aggregation || ""}
                  onValueChange={(value) =>
                    handleInputChange("aggregation", value)
                  }
                >
                  <SelectTrigger className="h-8 text-xs">
              ...
              <div>
                ...
                <TextField.Root
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                    handleInputChange("outputColumnName", e.target.value)
                  }
                />
              </div>
            </>
        ...
        <CardContent className="space-y-1 p-3 text-sm">
          <div>
            <strong>输入索引:</strong> {displayIncomingIndex}
          </div>
          <div>
            <strong>关联文件:</strong> {displayFileName}
          </div>
          <div>
            <strong>关联列:</strong> {data.inputIndexColumn || "N/A"}
          </div>
          <div>
            <strong>数据列:</strong> {data.dataColumn || "N/A"}
          </div>
          <div>
            <strong>计算:</strong>{" "}
            {aggregationOptions.find((o) => o.value === data.aggregation)
              ?.label ||
              data.aggregation ||
              "N/A"}
          </div>
          <div>
            <strong>输出为:</strong> {data.outputColumnName || "N/A"}
          </div>
        </CardContent>
      )}
