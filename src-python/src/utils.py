...
# Keep original name pointing to the single value serializer for compatibility if needed
...
def normalize_response(data) -> str:
    ...
    if hasattr(data, 'dict'): # Pydantic V1
        ...
        # This path might fail if data contains unserializable types
         try:
             return json.dumps(data, ensure_ascii=False, indent=2, default=serialize_value)
         except TypeError:
             raise TypeError(f"Cannot automatically serialize type {type(data)}. Convert to dict first.")
