...
class SheetSelectorOutput(NodeOutput):
    """表选择节点输出"""
    dataframe: DataFrame = Field(..., description="选中的DataFrame")
...
class RowFilterInput(NodeInput):
    """行过滤节点输入"""
    dataframe: DataFrame = Field(..., description="待过滤的DataFrame")
    index_value: IndexValue = Field(..., description="当前索引值")


class RowFilterOutput(NodeOutput):
    """行过滤节点输出"""
    dataframe: DataFrame = Field(..., description="过滤后的DataFrame")
    index_value: IndexValue = Field(..., description="对应的索引值")
...
class RowLookupInput(NodeInput):
    """行查找节点输入"""
    dataframe: DataFrame = Field(..., description="源DataFrame")
    index_value: IndexValue = Field(..., description="用于匹配的索引值")


class RowLookupOutput(NodeOutput):
    """行查找节点输出"""
    dataframe: DataFrame = Field(..., description="匹配的行组成的DataFrame")
    index_value: IndexValue = Field(..., description="对应的索引值")
...
class AggregatorInput(NodeInput):
    """聚合节点输入"""
    dataframe: DataFrame = Field(..., description="完整的上游非聚合节点输出DataFrame")
    index_value: IndexValue = Field(..., description="当前索引值")
...
class OutputInput(NodeInput):
    """输出节点输入"""
    branch_aggregated_results: Dict[str, Dict[IndexValue, Dict[str, Union[int, float, str, None]]]] = Field(
...
class GlobalContext(BaseModel):
    ...
    loaded_dataframes: Dict[str, pd.DataFrame] = Field(default_factory=dict, description="已加载的DataFrame缓存")
    execution_mode: ExecutionMode = Field(default=ExecutionMode.PRODUCTION, description="执行模式")
...
class PathContext(BaseModel):
    ...
    last_non_aggregator_dataframe: Optional[DataFrame] = Field(None, description="最近的非聚合节点输出DataFrame")
    current_dataframe: Optional[DataFrame] = Field(None, description="当前节点的DataFrame")
...
class BranchContext(BaseModel):
    ...
    branch_metadata: Dict[str, Any] = Field(default_factory=dict, description="分支元数据")
    ...
    def get_final_results(self) -> Dict[IndexValue, Dict[str, Union[int, float, str, None]]]:
        """获取最终聚合结果"""
        final_results = {}
        for index_value, results in self.aggregation_results.items():
            final_results[index_value] = {
                result.column_name: result.result_value 
...
class BranchExecutionResult(BaseModel):
    ...
    success: bool = Field(..., description="执行是否成功")
    final_aggregations: Dict[IndexValue, Dict[str, Union[int, float, str, None]]] = Field(
        ..., description="最终聚合结果"
    )
...
class PipelineExecutionResult(BaseModel):
    ...
    index_results: List[IndexExecutionResult] = Field(..., description="各索引值执行结果")
    branch_results: List[BranchExecutionResult] = Field(..., description="各分支执行结果")
...
def validate_node_input_output_compatibility(node_type: NodeType, input_data: NodeInput, output_data: NodeOutput) -> bool:
    """验证节点输入输出类型兼容性"""
    type_mapping = {
    ...
    return isinstance(input_data, expected_input) and isinstance(output_data, expected_output)
...
__all__ = [
    ...
    # 执行上下文类型
    "FileInfo", "GlobalContext", "PathContext", "BranchContext",
    ...
    # 最外层API类型
    "PipelineExecutionSummary", "PipelineExecutionResult",
    "WorkspaceConfig", "ExecutePipelineRequest",
