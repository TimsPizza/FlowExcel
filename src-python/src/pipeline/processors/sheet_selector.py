...
from ..models import (
    BaseNode, SheetSelectorInput, SheetSelectorOutput, DataFrame,
    GlobalContext, PathContext, BranchContext, NodeType
)


class SheetSelectorProcessor(AbstractNodeProcessor[SheetSelectorInput, SheetSelectorOutput]):
    """Sheet选择器节点处理器"""
    ...
    def process(
        ...
        if mode == "manual":
            if not manual_sheet_name:
                raise ValueError("manualSheetName is required when mode is 'manual'")
            ...
            # 根据列匹配模式选择sheet
            if not match_column:
                raise ValueError("matchColumn is required when mode is 'column_match'")
        ...
        # 加载DataFrame
        pandas_df = self.load_dataframe_from_file(global_context, target_file_id, sheet_name)
        
        # 应用测试模式限制
        # pandas_df = self.apply_test_mode_limit(pandas_df, global_context)
        
        # 转换为自定义DataFrame格式
        custom_df = DataFrame.from_pandas(pandas_df)
    ...
    def _find_sheet_by_column_match(
        ...
        for sheet_meta in file_info.sheet_metas:
            ...
            try:
                # 加载sheet并检查是否包含匹配值
                df = self.load_dataframe_from_file(global_context, file_info.id, sheet_name)
