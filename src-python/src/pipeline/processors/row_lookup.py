...
from ..models import (
    BaseNode, RowLookupInput, RowLookupOutput, DataFrame,
...
class RowLookupProcessor(AbstractNodeProcessor[RowLookupInput, RowLookupOutput]):
    ...
    def process(
        ...
        match_mode = data.get("matchMode", "exact")  # exact, contains, starts_with, ends_with
        ...
        matched_df = self._find_matching_rows(
            pandas_df, lookup_column, input_data.index_value, match_mode, case_sensitive
        )
        ...
        # 转换回自定义DataFrame格式
        custom_df = DataFrame.from_pandas(matched_df)
