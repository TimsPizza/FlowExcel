...
from ..models import (
    BaseNode, OutputInput, OutputResult, SheetData, DataFrame, IndexValue,
    GlobalContext, PathContext, BranchContext, NodeType, ExecutionMode
)
...
class OutputProcessor(AbstractNodeProcessor[OutputInput, OutputResult]):
    ...
    def process(
        ...
        for branch_id, branch_aggregations in input_data.branch_aggregated_results.items():
            sheet_data = self._create_sheet_for_branch(
                branch_id, branch_aggregations, include_index_column, index_column_name, 
                global_context, node_map, context_manager
            )
            sheets.append(sheet_data)
        
        # 如果是生产模式且指定了输出路径，则写入文件
        if global_context.execution_mode == ExecutionMode.PRODUCTION and output_file_path:
    ...
    def _create_sheet_for_branch(
        ...
        if not branch_aggregations:
            ...
            source_name = self._get_source_name_for_branch(branch_id, global_context, node_map, context_manager)
            return SheetData(
        ...
        try:
            ...
            if context_manager and node_map:
                ...
                if index_source_node_id in node_map:
                    ...
                    display_name = source_node.data.get("displayName") or source_node.data.get("label") or index_source_node_id
                    return display_name
