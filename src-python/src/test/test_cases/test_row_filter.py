...
class TestRowFilter(BaseTestFramework):
    """RowFilter节点测试类"""
    
    def __init__(self):
        super().__init__()
        self.node_type = NodeType.ROW_FILTER
    ...
    def run_tests(self) -> TestSuiteResult:
        ...
        # 加载测试规范
        test_specs = self.load_test_specs("row_filter.json")
        ...
        # 创建测试套件结果
        suite_result = TestSuiteResult(f"RowFilter ({len(test_cases)} 个测试)")
...
class TestRowFilterUnittest(unittest.TestCase):
    """RowFilter节点单元测试类 - 兼容unittest"""
    
    def setUp(self):
        self.test_framework = TestRowFilter()
    
    def test_all_row_filter_cases(self):
        """测试所有RowFilter测试用例"""
        ...
        self.assertEqual(
            ...
            f"{suite_result.failed_tests} 个RowFilter测试失败"
        )
