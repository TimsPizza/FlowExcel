"""
IndexSource节点测试类
从index_source.json加载测试用例并执行
"""

import unittest
from test_cases.test_base import BaseTestFramework, TestSuiteResult
from pipeline.models import NodeType


class TestIndexSource(BaseTestFramework):
    """IndexSource节点测试类"""
    
    def __init__(self):
        super().__init__()
        self.node_type = NodeType.INDEX_SOURCE
    ...
    def run_tests(self) -> TestSuiteResult:
        ...
        # 创建测试套件结果
        suite_result = TestSuiteResult(f"IndexSource ({len(test_cases)} 个测试)")
...
class TestIndexSourceUnittest(unittest.TestCase):
    """IndexSource节点单元测试类 - 兼容unittest"""
    
    def setUp(self):
        self.test_framework = TestIndexSource()
    
    def test_all_index_source_cases(self):
        """测试所有IndexSource测试用例"""
        ...
        self.assertEqual(
            ...
            f"{suite_result.failed_tests} 个IndexSource测试失败"
        )
