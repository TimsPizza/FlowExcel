...
class TestSheetSelector(BaseTestFramework):
    """SheetSelector节点测试类"""
    
    def __init__(self):
        super().__init__()
        self.node_type = NodeType.SHEET_SELECTOR
    ...
    def run_tests(self) -> TestSuiteResult:
        ...
        # 加载测试规范
        test_specs = self.load_test_specs("sheet_selector.json")
        ...
        # 创建测试套件结果
        suite_result = TestSuiteResult(f"SheetSelector ({len(test_cases)} 个测试)")
...
class TestSheetSelectorUnittest(unittest.TestCase):
    """SheetSelector节点单元测试类 - 兼容unittest"""
    
    def setUp(self):
        self.test_framework = TestSheetSelector()
    
    def test_all_sheet_selector_cases(self):
        """测试所有SheetSelector测试用例"""
        suite_result = self.test_framework.run_tests()
        
        # 断言所有测试都通过
        self.assertEqual(
            suite_result.failed_tests, 0,
            f"{suite_result.failed_tests} 个SheetSelector测试失败"
        )
        
        # 确保有测试用例执行
        self.assertGreater(
            suite_result.total_tests, 0,
            "没有找到SheetSelector测试用例"
        )


if __name__ == "__main__":
    # 直接运行测试
    test_runner = TestSheetSelector()
