...
class I18nMiddleware(BaseHTTPMiddleware):
    ...
    async def _localize_json_response(self, response: JSONResponse, language: str) -> JSONResponse:
        """本地化JSON响应中的消息"""
        try:
            # 获取响应内容
            response_body = b""
...
class LocalizedAPIResponse:
    ...
    @staticmethod
    def success(data: Any = None, message_key: str = None, language: str = 'zh', **message_params):
        """创建成功响应"""
        response_data = {
            "success": True,
            "data": data
        ...
        if message_key:
            response_data["message"] = get_localized_message(message_key, language, **message_params)
        
        return JSONResponse(content=response_data)
    
    @staticmethod
    def error(error_key: str, language: str = 'zh', status_code: int = 400, **error_params):
        """创建错误响应"""
