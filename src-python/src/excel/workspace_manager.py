...
class WorkspaceManager:
        ...
        try:
            for file_path in os.listdir(self.workspace_dir):
                if file_path.endswith(".json"):
                    try:
                        with open(self.workspace_dir + "/" + file_path, 'r', encoding='utf-8') as f:
                            config = json.load(f)
    ...
    def load_workspace(self, workspace_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if config.get('id') != workspace_id:
                raise ValueError(f"Workspace ID in file ({config.get('id')}) does not match requested ID ({workspace_id})")
    ...
    def save_workspace(self, workspace_id: str, config_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if config_data.get('id') != workspace_id:
            raise ValueError(f"Workspace ID in config ({config_data.get('id')}) does not match save ID ({workspace_id})")
